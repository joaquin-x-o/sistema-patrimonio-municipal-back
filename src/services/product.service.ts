...
import { calculateReviewDaysPending } from "../utils/productUtils/calculateReviewDaysPending";
...
export class ProductService {
    ...
    async createProduct(userId: number, dto: CreateProductDto): Promise<ProductResponseDto> {
        ...
        const newProduct = new Product({
            ...
            pendingReviewReason: dto.status === ProductStatus.IN_REVIEW ? dto.pendingReviewReason ?? null : null,
    ...
    // GET: obtener todos los productos + filtros
    async findAllProducts(page: number, limit: number, productSearch?: string, departmentCode?: string, statuses?: ProductStatus[]): Promise<{
        ...
        let finalStatuses: ProductStatus[];

        if (statuses && statuses.length > 0) {
            finalStatuses = statuses;
        } else {
            // en caso de no buscar productos por un estado en particular, 
            // por defecto se mostraran los que estan activos, en revisión y en desuso 
            finalStatuses = [
        ...
        // extracción de filtros dinámicos si los hay
        ...
        const [products, total] = await this.productRepository.searchProducts(page, limit, filters);

        const foundProducts = products.map(product => {
            const needsCheckReview = shouldBeChecked(product.lastCheckDate);
            const reviewDaysPending = calculateReviewDaysPending(product.status, product.statusUpdatedAt, needsCheckReview)
            ...
            responseDto.reviewDaysPending = reviewDaysPending;
    ...
    async findOneProductByCode(productCode: number): Promise<ProductResponseDto> {
        ...
        // se añade la validacion y los dias pendientes de revisión antes de enviar el DTO como respuesta        
        foundProduct.needsCheckReview = needsCheckReview;
        foundProduct.reviewDaysPending = reviewDaysPending;
    ...
    async updateProduct(productCode: number, dto: UpdateProductDto): Promise<ProductResponseDto> {
        ...
        Object.assign(currentProduct, dto)
    ...
    async reactivateProduct(productCode: number): Promise<ProductResponseDto> {
        ...
        // CREACION DEL PRODUCTO POR REACTIVAR
        Object.assign(currentProduct, {
    ...
    async sendProductToReview(productCode: number, dto: ReviewProductDto): Promise<ProductResponseDto> {
        ...
        if (currentProduct.status === ProductStatus.IN_REVIEW) {
    ...
    // PATCH: Reportar un producto como extraviado (Alerta temporal)
    async markProductAsLost(productCode: number): Promise<ProductShortResponseDto> {
        ...
        if (currentProduct.status === ProductStatus.LOST) {
            throw new AppError(`El producto ${productCode} ya se encuentra reportado como extraviado.`, 409);
        }

        const currentDate = todayAR();

        // MODIFICACION DEL PRODUCTO A PERDIDO
        Object.assign(currentProduct, {
        ...
        const reviewDaysPending = calculateReviewDaysPending(currentProduct.status, currentProduct.statusUpdatedAt, needsCheckReview)
    ...
    // PATCH: Reportar un producto extraviado como encontrado
    async markProductAsFound(productCode: number): Promise<ProductShortResponseDto> {
        ...
        if (currentProduct.status !== ProductStatus.LOST) {
            throw new AppError(`El producto ${productCode} no está reportado como extraviado.`, 409);
        }

        const currentDate = todayAR();

        // MODIFICACION DEL PRODUCTO ENCONTRADO (Pasa a revisión)
        Object.assign(currentProduct, {
            status: ProductStatus.IN_REVIEW,
            statusUpdatedAt: currentDate,
            pendingReviewReason: 'Producto recuperado tras extravío. Requiere de su inspección.',
        });
