...
export class RetirementHistoryService {
    ...
    async findAllRetirementHistoryEntries(page: number, limit: number, type?: RetirementType): Promise<{data: RetirementHistoryResponseDto[], meta: object}> {

        // busqueda de todas las entradas del historial
        const [entries, total] = await this.retirementHistoryRepository.searchRetirementHistoryEntries(page, limit, type);
