...
export class ProductController {
    ...
    async getProducts(req: Request, res: Response) {
        ...
        if (req.query.search) {
            productSearch = String(req.query.search);
        }

        let departmentCode: string | undefined;

        if (req.query.departmentCode) {
            departmentCode = String(req.query.departmentCode);
        }

        let statuses: ProductStatus[] | undefined;

        if (req.query.statuses) {

        const rawStatuses = Array.isArray(req.query.statuses)
            ? req.query.statuses
            : [req.query.statuses];
        ...
        const invalidStatuses = rawStatuses.filter(
            status =>
                !Object.values(ProductStatus)
                    .includes(status as ProductStatus)
        );

        if (invalidStatuses.length > 0) {
            throw new AppError(
                `Estado(s) inválido(s): ${invalidStatuses.join(', ')}`,
                400
            );
        }

        statuses = rawStatuses as ProductStatus[];
    }

        const products = await this.productService.findAllProducts(page, limit, productSearch, departmentCode, statuses);
    ...
    // PATCH: marcar un producto como extraviado
    async markProductAsLost(req: Request, res: Response) {

        const { productCode } = req.params;

        const product = await this.productService.markProductAsLost(Number(productCode));

        return res.status(200).json({ message: 'Producto marcado como extraviado', data: product });

    };

    // PATCH: marcar un producto como extraviado
    async markProductAsFound(req: Request, res: Response) {

        const { productCode } = req.params;

        const product = await this.productService.markProductAsFound(Number(productCode));

        return res.status(200).json({ message: 'Producto marcado como encontrado. Se envió a revisión.', data: product });
