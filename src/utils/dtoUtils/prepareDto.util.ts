...
export function prepareProductForShortDto(product: Product): object {
    const productForDto: any = { ...product };
...
export function prepareRetirementHistoryForDto(history: RetirementHistory) {
    ...
    // procesar department
    if (historyForDto) {
        historyForDto.department = {
            departmentCode: history.department.departmentCode,
            name: history.department.name,
            responsibleName: history.department.responsibleName
        };
    } else {
        historyForDto.department = null;
    }
